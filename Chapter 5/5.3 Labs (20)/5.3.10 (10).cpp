...
{
	return point.GetY() == (this->slope * point.GetX() + this->yIntercept);
}
...
int main(void)
{
	...
	std::cout << "Enter line begin point coordinates (x, y): ";
	std::getline(std::cin, input);

	splitedString = Split(input);
	Point2D point1(std::stod(splitedString[0]), std::stod(splitedString[1]));

	std::cout << "Enter line end point coordinates (x, y): ";
	std::getline(std::cin, input);

	splitedString = Split(input);
	Point2D point2(std::stod(splitedString[0]), std::stod(splitedString[1]));

	std::cout << "Enter point for checking (x, y): ";
	std::getline(std::cin, input);

	splitedString = Split(input);
	Point2D point3(std::stod(splitedString[0]), std::stod(splitedString[1]));

	Line2D line(point1, point2);

	std::cout << (line.Contains(point3) == 0 ? "not " : "") << "collinear" << std::endl;
